...
The `text` format needs a second opt-in. Set `enable_per_line_text: true` (which requires `enable_incremental_read`) to read text blobs incrementally, one log record per line. Without it, `text` stays on the legacy path and treats each whole blob as one log record. The `text` format accepts blobs of any extension (`.txt`, `.log`, extensionless) on either path; the structured formats require `.json` (or `.gz`).

> **Temporary flag.** `enable_incremental_read` exists so the incremental path can be adopted gradually. It will become the default and then be removed once the path is proven, at which point both the flag and the legacy behavior (including `text` as one whole-blob record) go away.
...
With the flags set, the `records-json`, `json`, and `text` formats are read **incrementally**. Some sources grow a blob in place instead of writing it once: Azure NSG/VNet flow logs, for example, write one `PT1H.json` per hour and append records to it every minute for the whole hour. For these formats the receiver reads only the bytes added since its last read, tracked per blob, so records appended after a blob first appears are still ingested, and already-consumed bytes are not re-read in normal operation (delivery is at-least-once).
...
| enable_incremental_read | bool | `false`             | `false`  | Opt into [incremental reading](#incremental-reading-of-append-growable-blobs) of append-growable blobs for the `records-json` and `json` formats (and `text` with `enable_per_line_text`). Off by default, every format parses each blob whole and dedupes by name. Temporary: this becomes the default and is then removed once the incremental path is proven. |
| enable_per_line_text | bool | `false`             | `false`  | Opt the `text` format into incremental, one-log-record-per-line reads. Requires `enable_incremental_read` and `blob_format: text` (config error otherwise). Off by default, a text blob is read whole as a single log record. |
...
| `text`         | Raw text. By default the whole blob becomes one log record. With `enable_per_line_text` (which requires `enable_incremental_read`), each non-empty line becomes its own log record and the blob is read incrementally.                                                                                                                  |
...
With this config (no `enable_per_line_text`), the whole blob becomes one log record with its contents as the string body. To emit one log record per line instead, set `enable_incremental_read: true` and `enable_per_line_text: true`; each non-empty line then becomes its own record (a trailing carriage return is stripped), and the same per-line handling applies whether or not the blob is gzip-compressed.
