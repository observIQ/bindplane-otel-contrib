...
func TestLogAdapter_EmptyBatch_NoPush(t *testing.T) {
	...
	a := NewLogAdapter(sink, Z3Config{}, Z3Config{}, false, zaptest.NewLogger(t))
...
func TestLogAdapter_ParseFunc_Success(t *testing.T) {
	sink := &consumertest.LogsSink{}
	a := NewLogAdapter(sink, Z3Config{}, Z3Config{}, true, zaptest.NewLogger(t))
...
func TestLogAdapter_ResourceAndRecordAttributes(t *testing.T) {
	sink := &consumertest.LogsSink{}
	resource := Z3Config{Base: map[string]any{
		"service.name": "blitz-test",
		"deployment":   "ci",
	}}
	attrs := Z3Config{Base: map[string]any{
		"log.source": "apache",
	}}
...
func TestLogAdapter_NilLogger_NoPanic(t *testing.T) {
	sink := &consumertest.LogsSink{}
	a := NewLogAdapter(sink, Z3Config{}, Z3Config{}, false, nil)
...
// and Metadata.Attributes merging over the receiver-config Z3 base.
...
func TestLogAdapter_PerRecordResource_MergesOverBase(t *testing.T) {
	...
	a := NewLogAdapter(sink, resource, Z3Config{}, false, zaptest.NewLogger(t))
	require.NoError(t, a.ConsumeLogs(context.Background(), []embed.LogRecord{{
		Message: "x",
		Metadata: embed.LogRecordMetadata{
...
func TestLogAdapter_PerRecordAttributes_MergeAndLock(t *testing.T) {
	...
	a := NewLogAdapter(sink, Z3Config{}, attrs, false, zaptest.NewLogger(t))
	require.NoError(t, a.ConsumeLogs(context.Background(), []embed.LogRecord{{
		Message: "x",
		Metadata: embed.LogRecordMetadata{
...
func TestLogAdapter_NilAndEmptyMetadata_Identical(t *testing.T) {
	// Q3: nil and empty Metadata maps both mean "no override".
	for _, meta := range []embed.LogRecordMetadata{
		{}, // nil maps
		{Resource: map[string]string{}, Attributes: map[string]any{}}, // empty maps
	} {
		sink := &consumertest.LogsSink{}
		resource := Z3Config{Base: map[string]any{"host.name": "h"}}
		attrs := Z3Config{Base: map[string]any{"a": "v"}}
		a := NewLogAdapter(sink, resource, attrs, false, zaptest.NewLogger(t))
		require.NoError(t, a.ConsumeLogs(context.Background(), []embed.LogRecord{{
			Message:  "x",
			Metadata: meta,
...
// Q1 resource grouping tests — records with distinct effective resources
...
func TestLogAdapter_ResourceGrouping_LockedKeyCollapsesGroups(t *testing.T) {
	// Q1 × Z3 interaction: when the differing key is locked, all
