...
// downstream consumer.
//
// The adapter holds per-receiver-entry Z3Config maps for the resource
...
// in blitz v0.18.0+) merge over the Z3 base maps with locked keys
...
// `ResourceLogs` (Q1 resource grouping); records that share a
...
//
// resource and attrs are Z3-parsed config maps (see Z3Config). Either
// may be a zero-value Z3Config; an empty config contributes no base
...
func NewLogAdapter(c consumer.Logs, resource, attrs Z3Config, parseBody bool, logger *zap.Logger) *LogAdapter {
	if logger == nil {
		logger = zap.NewNop()
	}
	return &LogAdapter{
...
//     `Metadata.Resource` over the adapter's `resource` Z3 base
...
//     over the adapter's `attrs` Z3 base (same locking semantics) and
