...
func TestBuildBlitzModules_RecipePath(t *testing.T) {
	...
	mods, err := buildBlitzModules(embed.TelemetrySettings{Logger: logger}, cfg, blitzConsumers{logs: nopBlitzLogConsumer{}})
	require.NoError(t, err)
...
`
	...
	_, err := buildBlitzModules(embed.TelemetrySettings{Logger: logger}, cfg, blitzConsumers{logs: nopBlitzLogConsumer{}})
	require.Error(t, err)
