...
**AWS Inferentia** (`inf1`, NeuronCore v1) is **unsupported, but appears to function.** A hardware test on an `inf1.xlarge` ran correctly. The receiver emitted the full runtime metric set (NeuronCore utilization, FLOPS, execution counts and latency, and memory) with correct resource attributes. We do not officially support it because AWS has put Inf1 on end-of-life. The Neuron SDK stopped shipping Inf1 AMIs and virtual environments at release 2.27. The Inf1 driver is frozen below version 2.21, and `torch-neuron` is archived. Two behaviors are specific to NeuronCore-v1. It reports no power telemetry, so `aws.neuron.device.power.utilization` is never emitted. Its ECC counters are present but always read zero.
...
2. On start, the receiver launches `neuron-monitor` (if available) and begins reading its JSON stream. It reads the sysfs tree on each scrape.
...
- If `neuron-monitor` (the primary path) is absent, fails to start, or its stream ends unexpectedly, the receiver logs a **single error** (not once per scrape) and continues on whatever the sysfs stream can provide. It does **not** crash the collector and does **not** repeatedly log the dead path.
...
This sysfs-sourced metric is **disabled by default**. It reports the device's power draw as a percentage of its maximum power. The receiver emits three statistics (`min`/`max`/`avg`) over the sampling period as a fraction (unit `1`), under the `aws.neuron.power.statistic` attribute. This mirrors the hostmetrics state-attribute pattern. AWS refreshes the values about once a minute (the averaging-window length is unspecified), so the reading lags load by roughly that interval. It reflects real power draw, rising and falling with the workload after that lag. It is therefore **power, not compute utilization**. Do not read it as NeuronCore busy-ness. It is off by default for that reason and because the receiver emits it only when the sysfs power status is `VALID` (idle or unsupported states emit nothing). The values come straight from the driver's sysfs node; the receiver does not synthesize them.
