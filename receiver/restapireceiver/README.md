...
| `headers`            | map       |         | `false`  | A map of custom headers to send with each request. Header values will appear in debug logs.                                                                                                    |
| `sensitive_headers`  | map       |         | `false`  | A map of custom headers with sensitive values (e.g., auth token ID). Values are masked in logs and debug output. Must not overlap with `headers`.                                              |
