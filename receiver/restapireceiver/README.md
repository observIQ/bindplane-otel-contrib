...
| `headers`            | map       |         | `false`  | A map of custom headers to send with each request. Header values may appear in debug logs; for sensitive credentials, use the dedicated `auth_mode` options instead.                           |
