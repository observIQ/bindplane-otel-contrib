...
| `sensitive_headers`  | map       |         | `false`  | A map of custom headers with sensitive values (e.g., auth tokens). Values are masked in logs and debug output. Must not overlap with `headers`.                                                |
