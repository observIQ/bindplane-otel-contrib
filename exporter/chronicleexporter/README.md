...
| `endpoint`                      | string            | `malachiteingestion-pa.googleapis.com` | `false`  | The endpoint for sending to Chronicle. Must not contain a `http://` or `https://` prefix. For `https`, the endpoint is combined with `location` (e.g. `chronicle.googleapis.com`). |
| `location`                      | string            |                                        | `https`  | The Chronicle region (e.g. `us`, `europe`). Required when `protocol` is `https`.                                                                           |
| `project`                       | string            |                                        | `https`  | The Google Cloud project ID. Required when `protocol` is `https`.                                                                                          |
...
| `creds_file_path`               | string            |                                        | `false`  | The file path to the Google credentials JSON file. Mutually exclusive with `creds`.                                                                        |
| `creds`                         | string            |                                        | `false`  | The Google credentials JSON. Mutually exclusive with `creds_file_path`.                                                                                    |
