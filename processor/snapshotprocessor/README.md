...
2. Whenever telemetry passes through the processor, the most recent records are copied into an in-memory buffer. The processor keeps a separate buffer per signal type, each holding roughly the most recent 100 log records, metric data points, or spans. Only the records the buffer will keep are copied, and once a buffer is full it admits about 100 new records per second, so on a busy pipeline most batches pass through untouched. A pipeline that sustains more than ten buffers' worth of records per second switches its buffer to on-demand mode: the buffer stops collecting (keeping its last snapshot, refreshed every ten seconds), and a request collects fresh records just in time, waiting up to one second; if the pipeline has gone quiet the request returns the last known snapshot. Telemetry is passed through to the next consumer unmodified. See `pkg/snapshot/PERFORMANCE.md` for measurements.
