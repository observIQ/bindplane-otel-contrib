...
// TestRaw_EmitsOriginalTextInsteadOfParsedStructure asserts raw mode skips parser
// selection. A JSON array then arrives as its original lines, which Chronicle needs.
func TestRaw_EmitsOriginalTextInsteadOfParsedStructure(t *testing.T) {
	...
	require.Equal(t, []any{
		...
		`  {"host":"b","msg":"second"}`,
		"]",
	}, bodies, "raw mode emits the object's lines verbatim")
}
